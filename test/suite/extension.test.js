...
suite('extension.js', () => {
  ...
  setup(() => {
    ...
    sinon
      .stub(generateTreeFunctions, 'generateTreeEverything')
      .callsFake(() => {});
  });
  ...
  test('should activate the extension and register commands', () => {
    ...
    assert.strictEqual(
      registerCommandStub.callCount,
      2,
      'Expected two commands to be registered'
    );
    assert.ok(
      ...
      'Expected generateTree command to be registered'
    );
    assert.ok(
      registerCommandStub.calledWith(
      ...
      'Expected generateTreeEverything command to be registered'
    );

    assert.strictEqual(
      contextStub.subscriptions.length,
      2,
      'Expected two commands to be added to subscriptions'
    );
  });
  ...
  test('should handle deactivation without issues', () => {
    ...
    assert.doesNotThrow(
      ...
      'Expected deactivate function to not throw an error'
    );
  });
});
